...
class DoctorScraper:
    ...
    def get_page(self, page_number):
        ...
        try:
            ...
            logger.error(f"Sayfa {page_number} alınırken hata: {e}")
            return None
    ...
    def scrape(self):
        ...
        for page in range(self.last_scraped_page + 1, self.end_page + 1):
            try:
                ...
                if not html_content:
                    logger.warning(f"Sayfa {page} alınamadı, atlıyor...")
                    continue
                ...
                if not doctors:
                    # Boş sayfa geldi, sayfalar bitmiş olabilir
                    logger.info(f"Sayfa {page} boş, tarama durduruluyor.")
                    break
        ...
        logger.info(f"Tarama tamamlandı. Toplam süre: {elapsed_time:.2f} saniye")
